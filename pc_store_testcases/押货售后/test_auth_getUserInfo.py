...
class TestClass:
    ...
    @allure.severity(P2)
    @allure.title("获取当前用户登录缓存信息-成功路径:85折押货退货下单时获取当前用户登录缓存信息检查")
    def test_auth_getUserInfo(self):
        
        returnNum = 2 # 退货数量
        getReason = None # 获取各种退换货原因
        searchPositiveProducts = None # 正库存商品编号
        mortgageReturn = None # 押货退货单id
        getUserInfo = None # 用户登录缓存信息
        ...
        def step_auth_getUserInfo():
            ...
            with _auth_getUserInfo(self.store_token_85) as r:
                assert r.status_code == 200            
                assert r.json()["code"] == 200
