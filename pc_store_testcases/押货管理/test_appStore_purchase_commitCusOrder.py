...
from setting import P1, P2, P3, username, store, name, productCode
...
@allure.story("/appStore/purchase/commitCusOrder")
class TestClass:
    """
    提交定制品押货单
    ...
    @allure.severity(P1)
    @allure.title("提交定制品押货单-成功路径: 提交定制品押货单检查")
    def test_appStore_purchase_commitCusOrder(self):
        ...
        def step_appStore_purchase_commitCusOrder():
            ...
            with _appStore_purchase_commitCusOrder(data, self.store_token) as r:
                assert r.status_code == 200            
                assert r.json()["code"] == 200
