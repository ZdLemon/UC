...
@allure.story("/mgmt/inventory/group-order/audit")
class TestClass:
    """
    审核团购单
    ...
    @allure.severity(P1)
    @allure.title("审核团购单-成功路径:审核不通过检查")
    def test_01_mgmt_inventory_group_order_audit(self):
    ...
    @allure.severity(P1)
    @allure.title("审核团购单-成功路径:审核通过检查")
    def test_02_mgmt_inventory_group_order_audit(self):
        ...
        def step_mgmt_inventory_group_order_audit():
            ...
            data = {
                "attachments":[], # 审核附件
                "auditResult": 1, # 审核结果：0、不通过 1、通过
                "auditView": f"我同意团购单{appendGroupOrder}申请", # 审核意见
                "orderNo": appendGroupOrder # 订单编号
            }
            with _mgmt_inventory_group_order_audit(data, self.access_token) as r: 
                assert r.status_code == 200
