    ...
    for store in store_listStore:
        ...
        if listStore is None:
            ...
            if clerk_page:
                ...
                if getMemberInfoById["cardStatus"] not in (0, 1) or getMemberInfoById["mobiles"]:
                    continue
                else:
...
@pytest.fixture(scope="session", autouse=True)
def login_store():
    "云商 30504555 登录店铺系统920240"
    r = _login(username=store, password="888888", channel="store").json()
...
@allure.title("给钱包加钱>=10000，以便钱包支付")
...
@allure.title("设置订单补报时间")
@pytest.fixture(scope="session", autouse=True)
def updateBaseMonthlyReport():
    "设置订单补报时间"
    
    from api.basic_services._auth_scenes_permission_updateBaseMonthlyReport import _auth_scenes_permission_updateBaseMonthlyReport
    ...
    with _auth_scenes_permission_updateBaseMonthlyReport(data, access_token) as r:
        assert r.status_code == 200
        assert r.json()["code"] == 200
        assert r.json()["data"] == "设置成功"

# 秒返券生效

@allure.title("使昨天的秒返券生效")
@pytest.fixture(scope="session", autouse=True)
def xxl_job_admin():
    "使昨天的秒返券生效"
    ...
    data = {
        "id": 109,
        "executorParam": {
            "startDate": f'{(datetime.date.today() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d")} 00:00:00', # 昨天"2022-04-07 00:00:00"
            "endDate": f'{(datetime.date.today() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d")} 23:59:59', # "2022-04-08 23:59:59"
        },
        "addressList": ""
    }
